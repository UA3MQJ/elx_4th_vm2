...
1.8 mem - работа с памятью

Операции `! @` работают и тестируются.

Для `variable` и `const` нужно дореализовать add_header. По сути, в core добавляю еще одно слово, но без ссылок на `module` и `function`, а в execute добавил доп проверку: если слово-переменная или слово-константа (то есть пустые поля), то ее не выполняем, как mfa, а исполняем так же, как интерпретируемое слово. Но тут, возможно, стоит еще подумать... Подумал - добавил типы в список core. До этого были только `CoreWord`, теперь еще `Constant` и `Variable`. У которых только имя и адрес.
